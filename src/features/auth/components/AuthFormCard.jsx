...
/**
 * AuthFormCard - Reusable form card component for authentication pages
...
export default function AuthFormCard({
    ...
    error,
    loading,
    image, // New prop for the side image
    reverse = false // Option to flip the layout (image on right)
}) {
    return (
        ...
        <div className="flex h-full flex-col lg:flex-row w-full max-w-6xl mx-auto bg-white rounded-lg shadow-xl overflow-hidden">
            {/* Image Side */}
            <div className={`hidden h-full lg:block lg:w-1/2 relative ${reverse ? 'lg:order-last' : ''}`}>
                <img
                    src={image}
                    alt="Auth background"
                    ...
                    {showGoogleSignIn && (
                        <div className="mt-6">
                            ...
                            >
                                ...
                                <span>Sign {title?.toLowerCase().includes('log') ? 'in' : 'up'} with Google</span>
                            </button>
                        </div>
                    )}
