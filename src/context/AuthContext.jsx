...
export function AuthProvider({ children }) {
  ...
  const updateProfile = async (updates) => {
    ...
    setUser(updated);
    const s =
      localStorage.getItem("td_session") ||
      sessionStorage.getItem("td_session");
    if (s) {
      const sess = JSON.parse(s);
      sess.user = updated;
      try {
        localStorage.setItem("td_session", JSON.stringify(sess));
      } catch (e) {
        console.error("persist session localStorage", e);
      }
      try {
        sessionStorage.setItem("td_session", JSON.stringify(sess));
      } catch (e) {
        console.error("persist session sessionStorage", e);
      }
    }
    return updated;
  };
  ...
  const setCart = (cart) => {
    if (!user) return;
    try {
      localStorage.setItem(`td_cart_${user.id}`, JSON.stringify(cart));
    } catch (e) {
  ...
  const addToCart = (item) => {
    ...
    if (existing)
      existing.qty = Math.min((existing.qty || 1) + (item.qty || 1), 99);
  ...
  const clearCart = () => {
    if (!user) return;
    try {
      localStorage.removeItem(`td_cart_${user.id}`);
    } catch (e) {
